...
async function renderDashboardFinancials() {
  ...
  const renderPanel = (stats) => {
    ...
    const projectsWithData = stats.projectsWithData || 0;
    const totalCostBasis = stats.totalCostBasis || 0;
    const totalCashPosition = stats.totalCashPosition || 0;
  ...
  if (typeof DB === 'undefined' || !(typeof isSupabaseReady === 'function' && isSupabaseReady())) {
    renderPanel({
  ...
  try {
    ...
    try {
      const projectRows = await DB.projects.getAll();
      if (Array.isArray(projectRows)) projects = projectRows;
    } catch (projectErr) {
      console.warn('Dashboard projects rollup unavailable:', projectErr);
    }
    ...
    try {
      const summaryRows = await DB.projectFinancialSummaries.getAll();
      if (Array.isArray(summaryRows)) summaries = summaryRows;
    } catch (summaryErr) {
      console.warn('Dashboard financial summary rollup unavailable:', summaryErr);
    }
        ...
        <div style="display:flex;align-items:flex-start;gap:12px">
          ...
          <div style="flex:1;min-width:0">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;flex-wrap:wrap">
              ...
              ${c.person ? `<span style="font-size:11px;color:var(--text-secondary)">&middot; ${escHtml(c.person)}</span>` : ''}
            </div>
            ...
            <div style="font-size:11px;color:var(--text-muted);margin-top:6px">By ${escHtml(c.createdBy)} &middot; ${new Date(c.createdAt).toLocaleString()}</div>
          </div>
...
function openCOEditor(idx) {
  ...
  var coCity = (typeof COMPANY !== 'undefined') ? [COMPANY.city, COMPANY.state].filter(Boolean).join(', ') || 'Portland, OR' : 'Portland, OR';
  var clientName = currentWO ? currentWO.client || '' : '';
  var clientAddr = currentWO ? currentWO.property || '' : '';
  var today = new Date().toLocaleDateString('en-US', {month:'2-digit', day:'2-digit', year:'numeric'});

  // Logo
  var logoHtml = '';
  if (typeof COMPANY !== 'undefined' && COMPANY.logo_url) {
    logoHtml = '<img src="' + COMPANY.logo_url + '" class="doc-logo-img">';
  } else {
    var initials = coName.split(' ').map(function(w){ return w[0]; }).join('').substring(0,3).toUpperCase();
    logoHtml = '<div class="doc-logo-initials">' + initials + '</div>';
  }

  // Line item rows -- same structure as renderDocLines()
  var rows = items.map(function(it, i) {
    var origTotal = it.original_price * it.original_qty;
    var curPrice = it.action === 'remove' ? 0 : it.new_price;
    var curQty = it.action === 'remove' ? 0 : it.new_qty;
    var lineTotal = curPrice * curQty;
    var isRemoved = it.action === 'remove';
    var rowStyle = isRemoved ? 'opacity:0.35;' : '';
    ...
    return '<tr id="co-row-' + i + '" style="' + rowStyle + '">' +
      ...
      '<td class="doc-td-total" id="co-lt-' + i + '">$' + lineTotal.toFixed(2) + '</td>' +
  ...
  container.innerHTML =
    ...
    '<div class="co-action-bar-center">' +
    '<span style="font-size:13px;font-weight:600;color:var(--text-secondary)">Template:</span>' +
    '<div style="display:flex;gap:6px">' +
    '<button class="doc-tpl-btn' + (_coTemplate === 'classic' ? ' active' : '') + '" id="co-tpl-btn-classic" onclick="setCOTemplate(\'classic\')">Classic</button>' +
    '<button class="doc-tpl-btn' + (_coTemplate === 'modern' ? ' active' : '') + '" id="co-tpl-btn-modern" onclick="setCOTemplate(\'modern\')">Modern</button>' +
    '<button class="doc-tpl-btn' + (_coTemplate === 'field' ? ' active' : '') + '" id="co-tpl-btn-field" onclick="setCOTemplate(\'field\')">Executive</button>' +
    '</div></div>' +
    '<button class="btn btn-sm btn-primary" onclick="saveCOFromEditor()" style="gap:6px"><i data-lucide="save" style="width:13px;height:13px"></i> Save</button>' +
    '</div>' +

    // ---- OFFICIAL DOCUMENT ----
    '<div id="co-document-wrap" class="doc-template-' + _coTemplate + '">' +
    '<div class="co-doc-inner">' +

    // Company Header
    '<div class="doc-header">' +
    '<div class="doc-logo-area">' + logoHtml + '</div>' +
    '<div class="doc-company-info">' +
    '<div class="doc-company-name">' + escHtml(coName) + '</div>' +
    '<div class="doc-company-detail">' + escHtml(coPhone) + '</div>' +
    '<div class="doc-company-detail">' + escHtml(coCcb) + '</div>' +
    '<div class="doc-company-detail">' + escHtml(coCity) + '</div>' +
    '</div></div>' +

    // Document Title Block (inside document body)
    '<div class="co-title-block">' +
    '<input type="text" id="co-ed-title" class="co-title-input" value="' + escHtml(co.title) + '" placeholder="Change Order Title">' +
    '<input type="text" id="co-ed-desc" class="co-desc-input" value="' + escHtml(co.description || '') + '" placeholder="Reason for change order...">' +
    '</div>' +

    // Meta Row (with Status + Requested By integrated)
    '<div class="doc-meta-row">' +
    '<div class="doc-meta-col"><div class="doc-meta-label">Prepared For</div>' +
    '<div class="doc-meta-value">' + escHtml(clientName || '\u2014') + '</div>' +
    '<div class="doc-meta-sub">' + escHtml(clientAddr || '\u2014') + '</div></div>' +
    '<div class="doc-meta-col"><div class="doc-meta-label">Date</div>' +
    '<div class="doc-meta-value">' + today + '</div></div>' +
    '<div class="doc-meta-col"><div class="doc-meta-label">Change Order #</div>' +
    '<div class="doc-meta-value">' + escHtml(co.coNumber) + '</div></div>' +
    ...
    '<div class="doc-total-row"><span>Original Scope Total</span><span id="co-ed-orig">$0.00</span></div>' +
    '<div class="doc-total-row"><span>Proposed Total</span><span id="co-ed-proposed">$0.00</span></div>' +
    '<div class="doc-total-row doc-grand-total"><span>NET IMPACT</span><span id="co-ed-impact" style="color:inherit">$0.00</span></div></div>' +

    // Signature Block
    '<div class="doc-signature-block">' +
    '<div class="doc-sig-col"><div class="doc-sig-line"></div><div class="doc-sig-label">Client Signature &amp; Date</div></div>' +
...
function setCOTemplate(tpl) {
  _coTemplate = tpl;
  var wrap = document.getElementById('co-document-wrap');
  if (wrap) wrap.className = 'doc-template-' + tpl;
  document.querySelectorAll('[id^="co-tpl-btn-"]').forEach(function(btn) {
    btn.classList.toggle('active', btn.id === 'co-tpl-btn-' + tpl);
  });
}
...
function coEditorToggleRemove(i) {
  ...
  var rEl = document.getElementById('co-rate-' + i); if (rEl) { rEl.disabled = isR; if (isR) rEl.value = '0.00'; }
...
function getPDFHTML(title, body, wo, docStyle) {
  ...
  if (docStyle === 'classic') {
    ...
    headerHTML =
      '<div class="report-header">' +
        ...
        '<div class="header-company">' +
          '<h1>' + COMPANY.name + '</h1>' +
          '<p>' + addressLine + ' &middot; ' + COMPANY.phone + ' &middot; ' + COMPANY.email + '</p>' +
        '</div>' +
      '</div>';
  } else {
    ...
    headerHTML =
      '<div class="report-header">' +
        ...
        '<div class="header-company">' +
          '<h1>' + COMPANY.name + '</h1>' +
          '<p>' + COMPANY.phone + '  &middot;  ' + COMPANY.email + '  &middot;  ' + addressLine + '</p>' +
        '</div>' +
      '</div>';
  }
  ...
  '<p>Generated by WOIMS &middot; Work Order &amp; Inspection Management System</p>' +
...
async function sendEmailNow() {
  ...
  try {
    var runtimeEnv = (typeof window !== 'undefined' && window.NEXARTWO_ENV) ? window.NEXARTWO_ENV : {};
    var runtimeSupabaseUrl = String(runtimeEnv.SUPABASE_URL || '').replace(/\/+$/, '');
    var emailFunctionUrl = typeof getSupabaseFunctionUrl === 'function'
      ? getSupabaseFunctionUrl('send-email')
      : (runtimeSupabaseUrl ? runtimeSupabaseUrl + '/functions/v1/send-email' : '');
...
function renderEmailStatus() {
  ...
  if (es.status === 'viewed' && es.viewedAt) {
    ...
    el.innerHTML = '<span style="font-size:11px;padding:3px 8px;border-radius:10px;background:#10b98122;color:#10b981;font-weight:600" title="Viewed by recipient &middot; ' + dateStr + '\nSent to: ' + escHtml(es.sentTo || '') + '">\uD83D\uDC41 Viewed &middot; ' + dateStr + '</span>';
  } else if (es.status === 'sent' && es.sentAt) {
    dateStr = new Date(es.sentAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
    el.innerHTML = '<span style="font-size:11px;padding:3px 8px;border-radius:10px;background:#10b98122;color:#10b981;font-weight:600" title="Delivered via Resend &middot; ' + dateStr + '\nTo: ' + escHtml(es.sentTo || '') + '">&#x2705; Sent &middot; ' + dateStr + '</span>';
  } else if (es.status === 'prepared' && es.sentAt) {
    dateStr = new Date(es.sentAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
    el.innerHTML = '<span style="font-size:11px;padding:3px 8px;border-radius:10px;background:#3b82f622;color:#3b82f6;font-weight:600" title="Opened in mail client &middot; ' + dateStr + '\nTo: ' + escHtml(es.sentTo || '') + '">&#x1F4E7; Email Prepared &middot; ' + dateStr + '</span>';
  } else {
    el.innerHTML = '';
  }
}
